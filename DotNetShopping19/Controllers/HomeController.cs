    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult FeaturedProducts()
        {
            try
            {
                ...
                .Select(x => new ProductBoxModel
                {
                    ...
                    CampaignName = x.Variant.Product.CampaignId == 0 ? "" : 
                    db.Campaigns.Where(c => c.CampaignId == x.Variant.Product.CampaignId).FirstOrDefault().Name
                })
                .ToList();
        ...
        public ActionResult BestSellerProducts()
        {
            try
            {
                ...
                .Join(db.Brands, oopvpc => oopvpc.oopvp.Product.BrandId, b => b.BrandId, (oopvpc, b) => new { oopvpc, Brand = b })
                ...
                x.oopvpc.oopvp.oopv.oop.Order.OrderDate > fromDate
                )
                .GroupBy(x => new {
                ...
                .Select(x => new ProductBoxModel
                {
                    ...
                    .OrderBy(i => i.Sequence).FirstOrDefault().FileName,
                    CampaignName = x.CampaignId == 0 ? "" : 
                    db.Campaigns.Where(c => c.CampaignId == x.CampaignId).FirstOrDefault().Name
                }).ToList();
                ViewBag.BestSellerProducts = newProducts;
            }
            catch (Exception ex)
            {
                ViewBag.BestSellerProducts = new List<ProductBoxModel>();
                ViewBag.Error = ex.Message;
            }
            return View();
        }
        ...
        public ActionResult Products(Int16? Category, Decimal? min, Decimal? max, Int16? Brand)
        {
            ...
            try
            {
                ...
                if (Category > 0)
                {
                    productsQuery = productsQuery.Where(x => x.Category.CategoryId == Category 
                    || x.Category.ParentId == Category);
                }
                productsQuery = productsQuery.Where(x => x.Variant.UnitPrice > min
                && x.Variant.UnitPrice < max);
        ...
        {
            ViewBag.Url = Request.Url.Authority;
