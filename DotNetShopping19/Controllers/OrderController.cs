...
namespace DotNetShopping.Controllers
{
    public class OrderController : Controller
    {
        ...
        [HttpPost]
        public ActionResult UpdateStatus(Int64 orderId, Order.OrderStatuses status, string shippingCode)
        {
            try
            {
                if (User.IsInRole("Admin") || User.IsInRole("Order Manager"))
                {
                    ...
                    if (order != null)
                    {
                        ...
                        if (shippingCode != "" && status == Order.OrderStatuses.Shipped)
                        {
                        ...
                        if (status == Order.OrderStatuses.Shipped)
                        {
                            ...
                            emailHelper.SendOrderShippedEmail(orderId, Request, Url, user);
                        }
